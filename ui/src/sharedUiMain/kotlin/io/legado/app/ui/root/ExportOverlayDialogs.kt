 ...
 *
 * 对照 app 端 `HandleFileContract.EXPORT → HandleFileDialog`: 导出前弹「保存到文件」,
 * 落盘成功后 `showExportSuccess` (路径编辑 + 确定复制)。桌面端书源/书架导出与规则类导出
...
/** 导出分发对话框核心 Composable (选项: 保存到文件), 供 Overlay 内容及规则导出复用。 */
@Composable
fun ExportDispatchDialog(
    ...
    // 防重复点击 (对照原版 HandleFileDialog: 点击不自动关闭, 等结果回传后 dismiss)
